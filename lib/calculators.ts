// Compatibility exports for gold utils
export const KARAT_MULTIPLIERS: Record<string, number> = {
  "24k": 1,
  "22k": 22 / 24,
  "21k": 21 / 24,
  "18k": 18 / 24,
  "16k": 16 / 24,
  "14k": 14 / 24,
  "12k": 12 / 24,
  "10k": 10 / 24,
};

export const UNIT_CONVERSIONS: Record<string, number> = {
  tola: 1,
  gram: 1 / 11.6638,
  "10g": 10 / 11.6638,
  ounce: 1 / 3.74976,
  kg: 1000 / 11.6638,
};

export const calculateGoldPrice = (
  basePricePerTolaPer24k: number,
  unit: string,
  karat: string,
): number =>
  Math.round(
    basePricePerTolaPer24k * UNIT_CONVERSIONS[unit] * KARAT_MULTIPLIERS[karat],
  );
export type TUnit = "Tola" | "Gram" | "Ounce" | "Kilogram";
export type TKarat = "24K" | "22K" | "21K" | "18K" | "14K" | "12K" | "10K";

export const UNITS: TUnit[] = ["Tola", "Gram", "Ounce", "Kilogram"];
export const KARATS: TKarat[] = [
  "24K",
  "22K",
  "21K",
  "18K",
  "14K",
  "12K",
  "10K",
];

export const TO_GRAMS: Record<TUnit, number> = {
  Tola: 11.6638,
  Gram: 1,
  Ounce: 31.1035,
  Kilogram: 1000,
};

export const KARAT_PURITY: Record<TKarat, number> = {
  "24K": 1,
  "22K": 22 / 24,
  "21K": 21 / 24,
  "18K": 18 / 24,
  "14K": 14 / 24,
  "12K": 12 / 24,
  "10K": 10 / 24,
};

export function sanitize(val: string): string {
  const parsed = parseFloat(val);
  if (Number.isNaN(parsed) || parsed <= 0) return "";
  return val;
}

export function formatPKR(amount: number): string {
  return new Intl.NumberFormat("en-PK", {
    style: "currency",
    currency: "PKR",
    maximumFractionDigits: 0,
  }).format(amount);
}

export function convertUnits(value: number, from: TUnit, to: TUnit): number {
  const grams = value * TO_GRAMS[from];
  return grams / TO_GRAMS[to];
}

export function formatNumber(num: number): string {
  if (Number.isNaN(num) || !Number.isFinite(num)) return "";
  return parseFloat(num.toPrecision(6)).toString();
}
